...
病院の感染対策ラウンドを、スマートフォンやタブレットでそのまま記録するための React + TypeScript 製 Web アプリです。担当者名を入力してラウンドを開始し、各チェックポイントごとに写真・場所名・コメントを登録し、最後に報告書を確認して Word ファイルとして保存できます。
...
6. 「レポート作成」から報告書を確認し、Word ボタンを押して保存します。
7. 下に出ている「アプリで開く」をクリックして、メールで送ります。iPhone から Mac へ送る場合は AirDrop も使えます。
