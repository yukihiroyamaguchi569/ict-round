...
- `docs/technical-spec.md`, `docs/user-guide.md`, and `docs/privacy-policy.md` are **published as public web pages** under `/docs/<slug>`, generated from the Markdown at build time by `scripts/build-docs.mjs`. Write them for an outside reader — a hospital's security review is a real audience. Never put internal notes, credentials, or unreleased plans in these three files. Other files under `docs/` are internal and are not published; publishing is controlled by the explicit list in that script.
