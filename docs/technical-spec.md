...
- 行の同一性は「項目 ID ＋ カテゴリ名 ＋ 項目の文言」で判定します。項目 ID は「カテゴリ名-連番」で機械生成されるため、別々に編集したチェックリストでは同じ ID が別の項目に割り当たり得ます。同じ ID でも文言が違う場合は別の行として出力し、1 つの報告書の中で項目 ID が重複していなければ、各部署の評価をその部署の文言の行に載せます。1 つの報告書の中で同じ ID が複数の項目に使われている場合は、評価がどの項目のものか判別できないため、最初に現れた項目の行に載せ、同じ ID の残りの行は `—` となります
...
- セルが `—` になるのは、その列（病棟）のどの報告書にも該当の項目が無い場合と、項目はあるが担当者全員が未評価の場合です。加えて、1 つの報告書の中で項目 ID が重複している場合は、最初に現れた項目以外の行も `—` になります
