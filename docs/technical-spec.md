...
- 行の並びは、読み込んだ全ファイルのチェック項目の和集合。列の並びは読み込み順で、画面上で入れ替えられます
...
- 列見出しは病棟名（未入力時は担当者名）。重複する場合は担当者名を併記し、それでも重なる場合は連番を付けます
- 評価は部署ごとに別の列として並べ、単一の値に集約しません。その部署に存在しない項目のセルは `—` と表示します
- 次の場合は画面に警告を表示します（出力は継続します）
  - 他ファイルにある項目を持たないファイルがある
  - 同じ項目 ID に別のチェック項目が割り当てられている（別々に作ったチェックリストが混在している場合。同じ項目のつもりでも行が分かれるため）
  - チェックリスト名が混在している
  - 部署数が 15 件を超え、部署の列が狭くなって読みにくくなる
- 出力は `docx` ライブラリで生成する A4 横向きの `.docx`。評価マトリクスに続けて、部署ごとの総評と写真を出力します
