...
- 行の同一性は「項目 ID ＋ カテゴリ名 ＋ 項目の文言」で判定します。項目 ID は「カテゴリ名-連番」で機械生成されるため、別々に編集したチェックリストでは同じ ID が別の項目に割り当たり得ます。同じ ID でも文言が違う場合は別の行として出力し、各部署の評価をその部署の文言の行に載せます
...
- セルが `—` になるのは、その列（病棟）のどの報告書にも該当の項目が無い場合と、項目はあるが担当者全員が未評価の場合です
- 次の場合は画面に警告を表示します（出力は継続します）
  ...
  - 1 つの報告書の中で同じ項目 ID が複数のチェック項目に使われている（該当する報告書と項目 ID を示します。評価は項目 ID で記録するため、一部の評価が本来の行に載らない可能性があります）
