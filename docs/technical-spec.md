...
読み込み時は `format` と `version` を検証し、想定外の内容は理由を示して読み込みを中止します。
...
- 入力は報告書の `.docx`。先頭バイトで ZIP かどうかを判定し、customXml パートから `RoundExport` を取り出します（ZIP でないファイルは、同じ `RoundExport` を表す JSON として解釈します）
